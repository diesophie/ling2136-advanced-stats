...
# looking at data
sim
# check lowest and highest value of Distance
sim |>
  group_by(Sem) |>
  summarize(min = min(Distance, na.rm = T),
            max = max(Distance, na.rm = T))
# remove NA's from data:
sim <- sim |>
  mutate(Distance = Distance, na.rm = T)

# desity plot of distribution of Distance across Sem 
sim|>
  # group by Sem
  group_by(Sem)|>
  # plot distrubution
  ...
  geom_density(aes(x = Distance),
               # adjust seethroughness of overlaping parts
               alpha = 0.4)+
  # add descriptive title and axis-description
  labs(title = "\nDistribution of Distance for semantically similar \nand semantically different cities",
       x = "Distance",
       y = "Density \n")+
  # adjust scale limits
...
plot_normal(mean = 108, sd = 35)

# after fitting a model with priors only, I adjusted the sd from 40 to 35, as 35 still coveres what is possible (0,214) but doesn't extend that much further
...
my_priors <- c(prior(normal(108, 35),
                     class = 'Intercept'),
               prior(normal(108, 35),
                     class = 'b',
                     coef = 'SemSimilar'))

# I choose to set the same prior for the two conditions as I dont have anything that makes it likely or shows stat there actually is a difference between semantically similar and different cities
...
xmdl <- brm(Distance ~ Sem,
            ...
            prior = my_priors,
            data = sim,
            # not sure if we should set a seed or not, but I'll do so
            seed = 42)
...
xmdl_prior_data |>
  # extract samples
  spread_draws(b_Intercept, b_SemSimilar) |>
  # seperate the two Sem categories by creating new columns
  mutate(Different = b_Intercept,
         Similar = b_Intercept + b_SemSimilar) |>
  # chaning the format to fit the data
  select(Different, Similar) |>
  pivot_longer(everything(), names_to = "Sem", values_to = "Distance") |>
  # plotting everything
  ggplot(aes(x = Distance,
  ...
  # add labels
  labs(title = "\nPosterior of Distance between cities in mm \npredicted by semantic category",
       x = "Distance (mm)",
       y = "Density") +
  # adjust scale
  # a bit to broad
...
# The data and the posterior match quite well. Hoeveer, at the peak, most of the light blue lines are below the peak of the data, showing that the posterior lowers the actual peak found in the data. Moreover, when it comes to the right tail/side of the distribution, the data has some bumps while the posterior is much smoother.
# This suggests, that the data actually isn't as normally distributed as out priors are set.
```
